...
namespace EmbedIO.Files
{
    ...
    public class FileModule : WebModuleBase, IDisposable, IMimeTypeCustomizer
    {
        ...
        private MappedResourceInfo? MapUrlPath(string urlPath, IMimeTypeProvider mimeTypeProvider)
        {
            ...
            if (RoutePaths.Count > 0 && RoutePaths.Contains(urlPath))
            {
                var routeDocumentPath = $"{urlPath}.html";
                var routeDocumentResult = Provider.MapUrlPath(routeDocumentPath, mimeTypeProvider);
                if (routeDocumentResult?.IsFile ?? false)
                    return routeDocumentResult;
            }
