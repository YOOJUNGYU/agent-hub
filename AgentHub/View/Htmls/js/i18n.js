...
(function (global) {
  ...
  const DICT = {
    ko: {
      ...
      'settings.hookAlways': '질문 알림 훅은 자동으로 설치됩니다(Claude가 입력을 기다리면 연결된 폰에 알림). 서버 시작 시 ~/.claude/settings.json에 항목을 추가하며 기존 훅은 보존됩니다.',
      // 모바일 — WebSocket 상태
      'ws.connecting': '연결 중…',
      ...
      'term.promptPh': '프롬프트·명령 입력 ( / 슬래시 명령도 가능)',
      'notify.on': '🔔 알림 켜기',
      'prompt.send': '보내기',
    ...
    en: {
      ...
      'settings.hookAlways': 'The question-alert hook is installed automatically (notifies your connected phone when Claude waits for input). It is added to ~/.claude/settings.json on server start; existing hooks are preserved.',
      ...
      'term.promptPh': 'Type a prompt or command ( / slash commands too)',
