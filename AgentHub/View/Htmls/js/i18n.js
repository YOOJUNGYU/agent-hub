...
(function (global) {
  ...
  const DICT = {
    ko: {
      ...
      'detail.back': '← 목록',
      'notify.on': '🔔 알림 켜기',
      'ask.title': 'AI 세션이 입력을 기다립니다',
      'elicit.title': 'AI 세션이 질문합니다',
      'elicit.question': '질문',
      ...
      'perm.title': 'AI 세션이 권한을 요청합니다',
      'perm.q': '실행을 허용할까요?',
      'perm.allow': '허용',
      'perm.deny': '거부',
      'perm.allowAlways': '허용 + 다음부터 안 물음',
      'perm.pendingTitle': '권한 대기 중 — 콘솔로 답하기',
      'perm.pendingHint': '라이브 응답 시간이 지났습니다. 아래 버튼으로 PC 터미널의 권한 프롬프트에 직접 답합니다.',
      'perm.injectFailed': '콘솔 주입에 실패했습니다. PC 터미널에서 직접 답해 주세요.',
      'guide.link': '📖 사용법',
      'pwa.install': '⬇ 앱 설치',
      ...
      'inject.send': '전송',
      'inject.hintNoConsole': '이 세션은 직접 입력이 안 되는 터미널(Windows Terminal 등)에서 실행 중입니다. cmd.exe 또는 PowerShell 창에서 claude/codex를 실행하면 모바일에서 바로 답할 수 있어요.',
      ...
      'inject.hintNotShell': 'PC에서 CLI(cmd·PowerShell·WSL 등)로 실행하고 훅이 연결된 Claude/Codex 세션에서만 모바일 답변을 입력할 수 있어요. 이 세션은 직접 입력을 받지 못합니다.',
    ...
    en: {
      ...
      'ask.title': 'AI session is waiting for your input',
      'elicit.title': 'AI session has a question',
      ...
      'perm.title': 'AI session is requesting permission',
      'perm.q': 'Allow this action?',
      'perm.allow': 'Allow',
      'perm.deny': 'Deny',
      'perm.allowAlways': 'Allow + don’t ask again',
      'perm.pendingTitle': 'Permission waiting — answer via console',
      'perm.pendingHint': 'The live response window has passed. Use the buttons below to answer the permission prompt in the PC terminal directly.',
      'perm.injectFailed': 'Console injection failed. Please answer in the PC terminal directly.',
      'guide.link': '📖 Guide',
      ...
      'inject.send': 'Send',
      'inject.hintNoConsole': 'This session runs in a terminal that does not accept direct input (e.g. Windows Terminal). Run claude/codex in a cmd.exe or PowerShell window and you can answer from mobile.',
      ...
      'inject.hintNotShell': 'Mobile replies work only for hooked Claude/Codex sessions started from a CLI (cmd, PowerShell, WSL, etc.) on the PC. This session does not accept direct input.',
