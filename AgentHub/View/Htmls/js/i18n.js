...
(function (global) {
  ...
  const DICT = {
    ko: {
      ...
      'tab.devices': '기기 관리',
      'tab.clients': '연결된 기기',
      'tab.sessionTerm': '세션 터미널',
      'tab.logs': '로그',
      'tab.settings': '설정',
      'sessionTerm.pick': '세션',
      'sessionTerm.open': '터미널 열기',
      'sessionTerm.hint': '폰에서 연 세션과 실시간으로 공유됩니다.',
      // 기기 관리
      ...
      'pending.offlineDesc': 'PC(Agent Hub)가 켜져 있는지 확인하세요. 인증서를 삭제했거나 새로 발급된 경우, 아래 주소를 휴대폰 브라우저(Chrome/Safari)로 열어 인증서를 다시 설치하세요(HTTP라 경고 없이 바로 받힙니다).',
      ...
      'askExpired.hint': '이 질문은 지금 앱에서 바로 답할 수 없습니다(원격 답변 시간이 지났거나 앱이 꺼져 있었음). 세션 터미널을 열어 직접 답하거나 PC에서 답해 주세요.',
      'askExpired.close': '닫기',
      'askExpired.openTerm': '⌨ 세션 터미널 열기',
      'detail.back': '← 목록',
      ...
      'cert.openInBrowser': '인증서를 삭제했거나 앱에서 안 열리면, 아래 주소를 휴대폰 브라우저로 여세요(HTTP라 경고 없이 바로 받힙니다):',
    ...
    en: {
      ...
      'tab.sessionTerm': 'Session terminal',
      'tab.logs': 'Logs',
      'tab.settings': 'Settings',
      'sessionTerm.pick': 'Session',
      'sessionTerm.open': 'Open terminal',
      'sessionTerm.hint': 'Shares live with the session opened on the phone.',
      ...
      'pending.offlineDesc': 'Make sure the PC (Agent Hub) is running. If you deleted or reissued the certificate, open the address below in your phone browser (Chrome/Safari) and install the certificate again (downloads over HTTP, no warning).',
      ...
      'askExpired.hint': "This question can't be answered from the app right now (the remote window passed, or the app was closed). Open the session terminal to answer directly, or answer on the PC.",
      'askExpired.close': 'Close',
      'askExpired.openTerm': '⌨ Open session terminal',
      'detail.back': '← List',
      'term.openSession': '⌨ Open terminal',
      'term.title': 'Terminal',
      'term.warn': 'This terminal runs real commands on the PC. Use only if you trust it.',
      'term.confirmOpen': 'The shell currently running this session will be closed and the session restarted. Run it from mobile and send prompts anyway?',
      'term.loading': 'Loading history…',
      ...
      'cert.openInBrowser': "If you deleted the certificate or it won't open in the app, open this address in your phone browser (downloads over HTTP, no warning):",
