...
(function (global) {
  ...
  const DICT = {
    ko: {
      ...
      'cert.ios': 'iOS: 다운로드 → 설정 앱의 “프로파일 다운로드됨” 열기 → 설치 → 설정 › 일반 › 정보 › 인증서 신뢰 설정에서 AgentHub 신뢰 켜기',
      'cert.android': 'Android: 다운로드 → CA 인증서로 설치 (설정 › 보안 › 인증서 설치 › CA 인증서)',
      'cert.note': '설치 후 새로고침하면 보안경고 없이 접속되고 홈 화면 추가(PWA 설치)가 가능합니다. iOS에서 설치 후에도 접속이 안 되면 인증서 유효기간 때문일 수 있습니다.',
      'summary.count': '세션',
    ...
    en: {
      ...
      'cert.ios': 'iOS: Download → open “Profile Downloaded” in Settings → Install → Settings › General › About › Certificate Trust Settings, enable trust for AgentHub',
      'cert.android': 'Android: Download → install as a CA certificate (Settings › Security › Install certificate › CA certificate)',
      'cert.note': 'After installing, refresh to connect without warnings and enable Add to Home Screen (PWA install). On iOS, if it still won’t connect after install, the certificate validity period may be the cause.',
