...
(function (global) {
  ...
  const DICT = {
    ko: {
      ...
      'notify.on': '🔔 알림 켜기',
      'session.new': '+ 새 세션',
      'session.newPrompt': '새 세션을 시작할 폴더 경로를 입력하세요',
      'session.external': '외부 실행 세션 — 보기 전용',
      'prompt.ph': '프롬프트 입력…',
      'prompt.send': '보내기',
    ...
    en: {
      ...
      'session.newPrompt': 'Enter the folder path to start a new session',
      'session.external': 'External session — view only',
      'prompt.ph': 'Type a prompt…',
      'prompt.send': 'Send',
